...
public class NettingTaskRepositoryAdapter implements NettingTaskRepositoryPort {
    ...
    public List<NettingTask> saveAll(List<NettingTask> tasks) {
        return repository.saveAll(tasks.stream().map(PersistenceMapper::toEntity).collect(Collectors.toList()))
                .stream()
    ...
    @Override
    public long maxSeq() {
        return repository.findTopByOrderBySeqDesc().map(e -> e.getSeq()).orElse(0L);
    }
}
